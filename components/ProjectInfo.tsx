import { Box, Button, Flex, Heading, Link,Image, Tag, Text, Spinner } from "@chakra-ui/react";
 ...
 const githubContent = (githubUrl:string, filename:string)=>{
    ...
    if(config.screenshots && !error){
     const images = config.screenshots.map((shot:string)=>{
        return path.join(`https://raw.githubusercontent.com/${username}/${repo}/${repoData.default_branch}/`, shot)
      })
...
//GITHUB CONTENT END
 ...
 <Box p="0">
   <Heading mb="10px" fontSize={"5xl"}>{name}</Heading>
       <Link href={github_url} target="_blank" fontSize={"sm"}>
          <Flex gap={2} align="center" textDecoration="underline">
              <SiGithub /><Text>Visit Github Page</Text>
          </Flex>
          </Link>
  <Link textDecoration="underline">
  <Flex  gap={2} align="center"  my="5px">  <FaUser />
  <Text> username </Text></Flex>
    </Link>
   ...
   <Flex m="12px 0">
    {loading && <Spinner />}
    {(!loading && !error && screenshots) && <Image src={screenshots[0]} objectFit="cover" bg={"grey.900"} w="full" h="350px" alt="screenshot" />}
   </Flex>
